...
async def create(request: Request, fields: CreateSensorModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session)):
...
async def update(request: Request, fields: UpdateSensorModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), sensor_id: int=0):
...
async def get_single(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), sensor_id: int = 0):
...
async def delete(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), sensor_id: int = 0):
