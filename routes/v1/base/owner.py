...
async def create(request: Request, fields: CreateOwnerModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session)):
...
async def create(request: Request, fields: UpdateOwnerModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), owner_id: int=0):
...
async def get_single(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), owner_id: int = 0):
