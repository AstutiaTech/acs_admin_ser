...
async def create(request: Request, fields: CreateControlBoxModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session)):
...
async def update(request: Request, fields: UpdateControlBoxModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), control_box_id: int=0):
...
async def get_by_asset(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), asset_id: int=0):
...
async def get_single(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), control_box_id: int = 0):
...
async def delete(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), control_box_id: int = 0):
