...
async def create(request: Request, fields: CreateInverterModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session)):
...
async def update(request: Request, fields: UpdateInverterModel, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), inverter_id: int=0):
...
async def get_single(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), inverter_id: int = 0):
...
async def delete(request: Request, admin=Depends(auth.auth_admin_wrapper), db: Session = Depends(get_session), inverter_id: int = 0):
