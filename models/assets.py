...
def create_asset(db: Session, owner_id: int=0, asset_type_id: int=0, reference: str=None, asset_type: int=0, name: str=None, description: str=None, address: str=None, city: str=None, state: str=None, country: str=None, latitude: str=None, longitude: str=None, status: int=0, created_by: int=0, updated_by: int=0):
    asset = Asset(owner_id=owner_id, asset_type_id=asset_type_id, reference=reference, asset_type=asset_type, name=name, description=description, address=address, city=city, state=state, country=country, latitude=latitude, longitude=longitude, status=status, created_by=created_by, updated_by=updated_by, created_at=get_laravel_datetime(), updated_at=get_laravel_datetime())
    db.add(asset)
    db.commit()
    db.refresh(asset)
    return asset
...
def get_assets_by_asset_type_id(db: Session, asset_type_id: int=0):
    return db.query(Asset).filter(and_(Asset.asset_type_id == asset_type_id, Asset.deleted_at == None)).order_by(desc(Asset.created_at))

def get_assets_by_asset_type_id_with_files(db: Session, asset_type_id: int=0):
    return db.query(Asset).join(Asset_File, Asset.id == Asset_File.asset_id).filter(and_(Asset.asset_type_id == asset_type_id, Asset.deleted_at == None, Asset_File.deleted_at == None)).options(joinedload(Asset.asset_files)).order_by(desc(Asset.created_at))

def get_assets_by_owner_id_and_asset_type_id(db: Session, owner_id: int=0, asset_type_id: int=0):
    return db.query(Asset).filter(and_(Asset.owner_id == owner_id, Asset.asset_type_id == asset_type_id, Asset.deleted_at == None)).order_by(desc(Asset.created_at))

def get_assets_by_owner_id_and_asset_type_id_with_files(db: Session, owner_id: int=0, asset_type_id: int=0):
    return db.query(Asset).join(Asset_File, Asset.id == Asset_File.asset_id).filter(and_(Asset.owner_id == owner_id, Asset.asset_type_id == asset_type_id, Asset.deleted_at == None, Asset_File.deleted_at == None)).options(joinedload(Asset.asset_files)).order_by(desc(Asset.created_at))
